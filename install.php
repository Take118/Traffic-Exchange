...
$sqlarray = array(
  ...
  value varchar(255) default NULL,  UNIQUE KEY `field` (`field`)
) ENGINE=MyISAM",
...
if (!isset($_POST['startinstall'])) {
  ...
  $db = @mysqli_connect($db_host, $db_user, $db_pwd) or die("Unable to connect to the database server - <b>" . $db->error . "</b>.<br>Please check database settings at vars.php file or<br><br><br> <a href='install.php'>Click Here</a> to Retry.");



  if (!@$db->select_db($db_name)) {

    exit("Unable to select the database $dbname - <b>" . $db->error . "</b>.<br>Please check database settings at vars.php file or<br><br><br> <a href='install.php'>Click Here</a> to Retry.");
  }
  ...
  foreach ($sqlarray as $sql) {
    ...
    if ($db->query($sql)) {
      ...
      echo "$t[0] $t[1] $t[2] Failed with Reason : " . $dvb->error() . "<br>";
    }
  }
